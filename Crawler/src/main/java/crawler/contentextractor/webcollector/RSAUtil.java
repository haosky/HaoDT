...
public abstract class RSAUtil {
    ...
    private static final String ALGORITHOMPADD = "RSA/ECB/PKCS1Padding";
    ...
    public static byte[] encrypt(byte[] byteData, String key){
    	try {
    		RSAPrivateKey rsaKey =getPrivateKey(key);
            Cipher cipher = Cipher.getInstance(ALGORITHOMPADD);
    ...
    public static String encrypt(String plaintext, String privateKey){
    	...
    	try {
    		...
    		byte[] en_data = cipher.doFinal(data);
    ...
    /***********
     * 公钥解密方法
     * @param encrypttext 待解密数据
     * @param publicKey 公钥
     * @return 解密后的数据
     */
    public static String decrypt(String encrypttext, String publicKey){
    	if (publicKey == null || encrypttext == null) {
            return null;
        }
        try {
        	byte[] en_data = Hex.decodeHex(encrypttext.toCharArray());
        	RSAPublicKey rsaKey =getPublicKey(publicKey);
            Cipher cipher = Cipher.getInstance(ALGORITHOMPADD);
            cipher.init(Cipher.DECRYPT_MODE, rsaKey);

            byte[] data =cipher.doFinal(en_data);
    ...
    public static byte[] decrypt(byte[] byteData, String key){
    	try {
    		RSAPublicKey rsaKey =getPublicKey(key);
    		Cipher cipher = Cipher.getInstance(ALGORITHOMPADD);
    		cipher.init(Cipher.DECRYPT_MODE, rsaKey);
