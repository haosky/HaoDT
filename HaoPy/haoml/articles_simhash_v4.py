...
class artcles_simhash():
    ...
    def get_es_row_with_count(self,article_src,num):
        ...
        search_body_str = self.query_table_data_dsl_row % (num, re.sub(u'\t|\r|[0-9]|[一二三四五六七八九十百千\（\）万元、：]','',article_src.strip()))
    ...
    def search_es_sim(self,sen_id,project,doc_phase,sentences_count):
        ...
        print 'es end time ' + str(es_end_time - es_start_time)
        src_docs_sim = []
        src_set = {}
        ...
        for src_doc in search_es_list:
            try:
                ...
                log.info(src_doc_id)
                if src_doc_id == docid:
                    print 'src_doc_id same ' + src_doc_id
                    continue
                doc_body = src_doc['_source']
                if 'content' not in doc_body:
                    print 'no content '+ src_doc_id
                    continue
            ...
            print 'sim_index_time ' + str(sim_index_end - sim_index_start)
            if len(sim_sames) == 0:
                continue
    ...
    def main_calc(self,search_uuid,suuid = None):
        ...
        print json.dumps(split_doc_phases,ensure_ascii=False)
        ...
        for sen_item in  self.build_queue_spout_unit(split_doc_phases,search_uuid,input_doc['project']):
            try:
               rlist.append(pool.apply_async(phase_detail, args = (sen_item,)))
            except:
                print traceback.format_exc()
        pool.close()
        pool.join()
        ...
        print(start)
        print 'multprocess start time %s' % multiprocessingstarttime
        print 'multprocess end time %s' % multiprocessingendtime
        print 'calc start time %s' % calc_time_start
        print(end)
...
def phase_detail(sen_item):
    try:
        ...
        print 'search_and_sim_time ' + str(search_and_sim_end_time - search_and_sim_start_time)
        return doc_data
    except:
        print traceback.format_exc()
...
if __name__ == '__main__':
          am = artcles_simhash()
          doc = am.get_es_id('cf682abac0d14fb1a944998e15bc4441')
          # print json.dumps(am.search_same_esart(doc['_source']['project'],doc['_source']['content']),ensure_ascii=False)
