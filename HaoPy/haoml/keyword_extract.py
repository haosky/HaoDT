...
class keyword_extract():
    ...
    def openHanlp(self):
        if not jpype.isJVMStarted():
            jpype.startJVM(jpype.getDefaultJVMPath(),
                     u"-Djava.class.path=%s/hanlp-1.3.3/hanlp-1.3.3.jar;%s/hanlp-1.3.3" % (hanlp_data,hanlp_data),
                     "-Xms128m",
    ...
    def hanlp_cut(self,content):
        self.openHanlp()
        content = unicode(content.strip())
        rs = str(self.HanLP.newSegment().enableAllNamedEntityRecognize(True).seg(content))
        self.closeHanlP()
        return rs

    def hanlp_cut_entry(self,content):
        m = str(self.hanlp_cut(content))
        das = m[1:-1].split(', ')
        result = []
        for d in das:
            wsplit = d.split('/')
    ...
    def extract_keywords(self,content,num):
        al = list(analyse.extract_tags(content,num))
        return al

def sub_hanlp_entry_cut(content,queue):
    try:
        ke = keyword_extract()
        ke.openHanlp()
        result = ke.hanlp_cut_entry(content)
        queue.put(result)
        # ke.closeHanlP()
        return
    except:
...
def sub_hanlp_cut(content,queue):
    try:
        ke = keyword_extract()
        ke.openHanlp()
        result = ke.hanlp_cut(content)
        queue.put(result)
        # ke.closeHanlP()
        return
    except:
...
def process_hanlp_entry_cut(content):
    q = Queue()
    prc = Process(target=sub_hanlp_entry_cut,args=(content,q))
    prc.start()
    rs = q.get()
    prc.join()
    q.close()
...
def process_hanlp_cut(content):
    q = Queue()
    prc = Process(target=sub_hanlp_cut,args=(content,q))
    prc.start()
    rs = q.get()
    prc.join()
    q.close()
